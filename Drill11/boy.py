...
# Boy Event
RIGHT_DOWN, LEFT_DOWN, RIGHT_UP, LEFT_UP, SLEEP_TIMER, SHIFT_DOWN, SHIFT_UP, SHIFT_TIME_OUT = range(8)
...
class DashState:
    ...
    def do(boy):
        ...
        if boy.timer == 0:
            if boy.velocity > 0:
                boy.add_event(SHIFT_TIME_OUT)
            if boy.velocity < 0:
                boy.add_event(SHIFT_TIME_OUT)
        boy.x += boy.velocity * 8
        boy.x = clamp(25, boy.x, 800 - 25)
...
next_state_table = {
    ...
    DashState: {SHIFT_UP: RunState, SHIFT_TIME_OUT: RunState}
}
